...
year_cluster_slider_marks = {year: str(year) for year in range(1974, 2019, 5)}
ncluster_cluster_slider_marks = {n: str(n) for n in range(2, 16)}
cluster_indicator_dropdown_options = [{'label': indicator, 'value': indicator} for indicator in poverty.columns[3:54]]
hist_indicator_dropdown_options = [{'label': indicator, 'value': indicator} for indicator in poverty.columns[3:54]]
hist_multi_year_selector_options = [{'label': year, 'value': year} for year in
                                    poverty['year'].drop_duplicates().sort_values()]
hist_bins_slider_marks = {x: str(x) for x in range(0, 105, 5)}
...
database = 'https://datacatalog.worldbank.org/dataset/poverty-and-equity-database'
